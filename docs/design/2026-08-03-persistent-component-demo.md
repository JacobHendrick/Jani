...
1. **Does the device honor the barrier?** Hardware. Needs a real machine and a
   real power cut. Hardware cannot be verified automatically (per docs/development/devlog.md), so this
...
Silent-failure, judgment-heavy, or permanent work:
...
Mechanical, spec-driven work with loud failures:
...
| Spec, plan, devlog, docs/development/devlog.md updates | Docs |

Every change receives review.
