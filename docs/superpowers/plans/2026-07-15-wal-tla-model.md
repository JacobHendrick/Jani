...
- Spec doc (authority): `docs/superpowers/specs/2026-07-15-wal-tla-model-design.md`
...
- Task 4 is written BY THE USER (Jacob) — the executor scaffolds it and stops; do not fill in the TODO blocks
...
### Task 4: USER-WRITTEN — the Crash action and NoTornVersionVisible

**STOP: this task's TLA+ is written by Jacob, not the executor.** The executor's job here is only to verify the preconditions (Task 3 committed, TODO markers present) and hand off. These ~12 lines define what "crash" and "torn" MEAN — the two definitions the whole verification rests on.
...
Expected: `Model checking completed. No error has been found.` — now over the FULL crash-recovery state space (tens of thousands of states, up to a few minutes). If TLC reports a violation, read the counterexample trace bottom-up: either the `Crash`/invariant text needs adjusting, or TLC has genuinely caught a protocol hole — both outcomes are the tool doing its job. Use superpowers:systematic-debugging if the trace is surprising.
...
**Files:**
- Modify: `docs/superpowers/specs/2026-07-15-wal-tla-model-design.md` (mutant #3 rename)
