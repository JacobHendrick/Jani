...
This was Jani's first driver and established the project's driver style. The
hosted virtqueue harness and Makefile wiring followed the interfaces recorded
below.
...
put, snapshot, mutate, rollback, and a clean remount on a second boot). It
still needs hardware verification. The prose below is kept as the
