...
test: $(TEST_PMM_BIN) $(TEST_HEAP_BIN) $(TEST_OBJECT_TABLE_BIN) $(TEST_CAPABILITY_BIN) $(TEST_OBJECT_HEADER_BIN) $(TEST_WAL_BIN) $(TEST_OBJECT_STORE_BIN) $(TEST_WRITE_ORDERING_BIN) $(TEST_WASM_SHIM_BIN) $(TEST_WASM_MODULE_BIN) $(TEST_SYSCALL_ARGS_BIN) $(TEST_COMPONENT_STATE_BIN) $(TEST_COMPONENT_MAILBOX_BIN) $(TEST_COMPONENT_UNINSTALL_BIN) $(TEST_IDLC_BIN) $(TEST_DETERMINISM_BIN)
	$(TEST_PMM_BIN)
	$(TEST_HEAP_BIN)
...
$(TEST_CAPABILITY_BIN): $(CAPABILITY_SOURCE) $(OBJECT_ID_SOURCE) $(HOSTED_DIR)/test_capability.c $(HOSTED_DIR)/check.h Makefile
	mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CFLAGS) $(CAPABILITY_SOURCE) $(OBJECT_ID_SOURCE) $(HOSTED_DIR)/test_capability.c -o $(TEST_CAPABILITY_BIN)
...
$(TEST_COMPONENT_MAILBOX_BIN): $(COMPONENT_SOURCE) $(INSTANCE_STATE_SOURCE) $(OBJECT_ID_SOURCE) $(OBJECT_TABLE_SOURCE) $(OBJECT_STORE_SOURCE) $(STRING_SOURCE) $(OBJECT_HEADER_VALIDATE_HOSTED_OBJ) $(WAL_VALIDATE_HOSTED_OBJ) $(HOSTED_DIR)/test_component_mailbox.c $(HOSTED_DIR)/check.h kernel/wasm/component.h Makefile
	mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CFLAGS) $(COMPONENT_SOURCE) $(INSTANCE_STATE_SOURCE) $(OBJECT_ID_SOURCE) $(OBJECT_TABLE_SOURCE) $(OBJECT_STORE_SOURCE) $(STRING_SOURCE) $(HOSTED_DIR)/test_component_mailbox.c $(OBJECT_HEADER_VALIDATE_HOSTED_OBJ) $(WAL_VALIDATE_HOSTED_OBJ) -o $(TEST_COMPONENT_MAILBOX_BIN)

$(TEST_COMPONENT_UNINSTALL_BIN): $(COMPONENT_SOURCE) $(INSTANCE_STATE_SOURCE) $(OBJECT_ID_SOURCE) $(OBJECT_TABLE_SOURCE) $(OBJECT_STORE_SOURCE) $(STRING_SOURCE) $(OBJECT_HEADER_VALIDATE_HOSTED_OBJ) $(WAL_VALIDATE_HOSTED_OBJ) $(HOSTED_DIR)/test_component_uninstall.c $(HOSTED_DIR)/check.h kernel/wasm/component.h Makefile
	mkdir -p $(BUILD_DIR)
	$(HOST_CC) $(HOST_CFLAGS) $(COMPONENT_SOURCE) $(INSTANCE_STATE_SOURCE) $(OBJECT_ID_SOURCE) $(OBJECT_TABLE_SOURCE) $(OBJECT_STORE_SOURCE) $(STRING_SOURCE) $(HOSTED_DIR)/test_component_uninstall.c $(OBJECT_HEADER_VALIDATE_HOSTED_OBJ) $(WAL_VALIDATE_HOSTED_OBJ) -o $(TEST_COMPONENT_UNINSTALL_BIN)
...
$(SYSCALLS_OBJ): $(SYSCALLS_SOURCE) kernel/wasm/syscalls.h kernel/wasm/component.h Makefile
	mkdir -p $(BUILD_DIR) $(ZIG_GLOBAL_CACHE_DIR) $(ZIG_LOCAL_CACHE_DIR)
